...
- CDPA has one global `MAINTAINERS` role for CDP 9222. It belongs to no team and is never a normal route. Maintainers returns one inline report plus one allowlisted operational decision; it never edits source, tests, requirements, role reports, task deliverables, or task manifests, and the worker alone validates and applies its decision.
